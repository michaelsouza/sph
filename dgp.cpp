...
double dgp_t::maxRelRes( double* x ) {
    double maxRelRes = 0.0;
    int k, j;
    double *xi, *xj, dij, lij, uij, relres_ij;
    ...
    for ( int i = 0; i < m_nnodes; i++ ) {
       ...
       for ( k = m_i[ i ]; k < m_i[ i + 1 ]; k++ ) {
          ...
          relres_ij = 0.0;
          if ( dij < ( lij = m_l[ k ] ) ) {
             relres_ij = ( lij - dij ) / lij;
          } else if ( dij > ( uij = m_u[ k ] ) ) {
             relres_ij = ( dij - uij ) / uij;
          }

          if ( maxRelRes < relres_ij ) {
             maxRelRes = relres_ij;
          }
       }
    }
